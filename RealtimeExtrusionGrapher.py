        ...
        def getDataFields():
            try:                                    #cboValue not yet defined at this stage
                if cboValue == "4.5F Inner Jacket":
                    ...
                    staticVariables.LCL = 1.420
                    staticVariables.UCL = 1.450
                    staticVariables.operator = entry1.get()
                    staticVariables.lotNo = entry2.get()
...
def checkConnection():
    try:
        ...
        arduino = serial.Serial('/dev/ttyUSB0',19200,bytesize=serial.EIGHTBITS,parity=serial.PARITY_EVEN,stopbits=serial.STOPBITS_ONE, timeout=5)
            ...
            try:
                if (self.connectionError == False and self.permissionError == False):
                    if self.count < 1:  #connect once
                        self.adruino = serial.Serial('/dev/ttyUSB0',19200,bytesize=serial.EIGHTBITS,parity=serial.PARITY_EVEN,stopbits=serial.STOPBITS_ONE, timeout=5)
                        print('connected')
                        self.count = 1   
                    serialcmd = 'g210\r\n'
