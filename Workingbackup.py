...
def setPath():
    staticVariables.localPath = '%s/%s_%s.csv' %('C:/Users/Admin/Dropbox/Python/DynamicGraphing', staticVariables.lotNo,staticVariables.operator)
            ...
            try:
                ...
                while True:
                    ...
                    #on login page and no connection error
                        ...
                        ani = animation.FuncAnimation(staticVariables.fig, staticVariables.aniObj.animate, interval = 400)
                        break
...
def UserMessage(var):
    ...
    if staticVariables.userResult == True and type == 'stop':
        p = psutil.Process(staticVariables.pid)
        p.terminate()
...
class UserPrompt(object):
    ...
    def userAction(self,q):
        ...
        #print('home process is' + str(self.pid))
        if self.type == 'home':
            messagebox.showwarning('Caution', 'Stop graph before returning to home screen')
            p.terminate()   #terminate UserPrompt.userAction process
        else:
