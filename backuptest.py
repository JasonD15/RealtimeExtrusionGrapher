...
#src_path = 'C:/Users/jason.dichoso/Dropbox/Python/DynamicGraphing/ExportedGraphs/csvFolder'
#dest_path = 'C:/Users/jason.dichoso/Dropbox/Python/DynamicGraphing/ExportedGraphs/RdriveFolder'

#src_path = 'C:/Users/jason.dichoso/Dropbox/Python/DynamicGraphing/csvFolder'  #targets folder containing all csv files
...
class staticVariables(object):
    ...
    error = None
    pid = 0
    src_path = 'C:/Users/Admin/Dropbox/Python/DynamicGraphing/csvFolder'  #targets folder containing all csv files
    dest_path = 'C:/Users/Admin/Dropbox/Python/DynamicGraphing/RdriveFolder'
    btnState = False
    homeMsg = None
    xList = []
    ...
    UCL_list = []
    LCL_list = []
    xListdeq = deque(maxlen=1200)
    yListdeq = deque(maxlen=1200)
    UCL_listdeq = deque(maxlen=1200)
    LCL_listdeq = deque(maxlen=1200)
    index = 0
    initPos = 0
...
class Animate(object):    #animate 6F object
    ...
    def __init__(self, xInterval, ucl, lcl, usl, lsl):  
        ...
        staticVariables.initPos = 0
        xInterval_val = xInterval
        ucl_val = ucl
        lcl_val = lcl
        usl_val = usl
        lsl_val = lsl
        #staticVariables.xListdeq = deque(maxlen=xInterval_val) #y-axis collection
        #staticVariables.yListdeq = deque(maxlen=xInterval_val)
        #staticVariables.UCL_listdeq = deque(maxlen=xInterval_val)
        #staticVariables.LCL_listdeq = deque(maxlen=xInterval_val)
    ...
    def animate(self, i): #i for 'interval'
        if staticVariables.isPaused == True:
            ...
            lines = graph_data.split('\n')  #each row has an extra blank value for some reason list = ['1,2','2,3,'','']
            staticVariables.initPos = 0
            staticVariables.index = (len(lines) - 2)              #index of latest reading    
            try:
                for i in range(staticVariables.initPos,  staticVariables.index+1):
                    x,y = lines[staticVariables.index].split(',')   
                    staticVariables.xListdeq.append(str(x))
                    staticVariables.yListdeq.append(str(y))
                    staticVariables.UCL_listdeq.append(ucl_val)
                    staticVariables.LCL_listdeq.append(lcl_val)
                staticVariables.initPos = staticVariables.index
                ...
                self.xlist = staticVariables.xListdeq
                self.ylist = staticVariables.yList
                self.uclList = staticVariables.UCL_list
                self.lclList = staticVariables.LCL_list

                staticVariables.ax1.clear()
                #print(str(len(staticVariables.xListdeq)) + ' ' + str(len(staticVariables.yListdeq)))
                print(str(staticVariables.yListdeq))
                staticVariables.ax1.plot(staticVariables.xListdeq, staticVariables.yListdeq)
                ...
                staticVariables.ax1.plot(staticVariables.xListdeq, staticVariables.UCL_listdeq, linewidth=1, label='UCL', color='r')  
                staticVariables.ax1.plot(staticVariables.xListdeq, staticVariables.LCL_listdeq, linewidth=1, label='LCL', color='r')
...
class AnimateEntireGraph(Animate):    #animate 6F object
    ...
    def __init__(self, ucl, lcl, usl, lsl):  
        ...
        staticVariables.xList = []
        staticVariables.yList = []
        staticVariables.UCL_list = []
        staticVariables.LCL_list = []
    ...
    def animate(self, j): #i for 'interval'
        if staticVariables.isPaused == True:
            ...
            try:
                for i in range(staticVariables.initPos,  staticVariables.index+1):
                    x,y = lines[staticVariables.index].split(',')   
                    staticVariables.xList.append(str(x))
                    staticVariables.yList.append(str(y))
                    staticVariables.UCL_list.append(ucl_val)
                    staticVariables.LCL_list.append(lcl_val)
                staticVariables.initPos = staticVariables.index
                staticVariables.currentYReading = str(y)
                staticVariables.currentXReading = str(x)
                self.xcoord = ('%.3f' % float(staticVariables.currentXReading))
                ...
                self.ucl_ycoord = ('%.3f' % float(ucl_val))
                staticVariables.ax1.clear()
                print(len(staticVariables.yList))
                staticVariables.ax1.plot(staticVariables.xList, staticVariables.yList)
                staticVariables.ax1.set_title(staticVariables.operator + ' | ' + staticVariables.lotNo + ' | ' + staticVariables.cableType, size = 20)
                staticVariables.ax1.set_xlabel('Time (s)', size = 20, weight ='bold')
                staticVariables.ax1.set_ylabel('Outer diameter (mm)', size = 20, weight ='bold')
                staticVariables.ax1.plot(staticVariables.xList, staticVariables.UCL_list, linewidth=1, label='UCL', color='r')  
                staticVariables.ax1.plot(staticVariables.xList, staticVariables.LCL_list, linewidth=1, label='LCL', color='r')
                #staticVariables.ax1.legend(bbox_to_anchor=(1.005, 1), loc=2, borderaxespad=0.)
                staticVariables.ax1.annotate('OD = ' + str(self.ycoord), xy = (self.xcoord, self.ycoord),fontsize=20)
                staticVariables.ax1.annotate('LCL = ' + str(self.lcl_ycoord), xy = (self.xcoord, self.lcl_ycoord),fontsize=20)
                staticVariables.ax1.annotate('UCL = ' + str(self.ucl_ycoord), xy = (self.xcoord, self.ucl_ycoord),fontsize=20)
            ...
            try:
                if (self.connectionError == False and self.permissionError == False):
                    if self.count < 1:  #connect once
                        self.adruino = serial.Serial('COM4', 9600, timeout=5)
                        print('connected')
                        self.count = 1   
                    serialcmd = 'g210'
