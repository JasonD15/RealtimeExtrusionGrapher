...
def setPath():
    ...
    #staticVariables.localPath_temp = '%s/%s_%s.csv' %('C:/Users/jason.dichoso/Dropbox/Python/DynamicGraphing/DynamicGraphing', staticVariables.lotNo,(staticVariables.operator+'_temp'))
...
class FixedInterval(tk.Tk):  #DynamicPage inherits from Frame class. DynamicPage 'is a' Frame
    def __init__(self): #override the frame cosntructor
        ...
        def switchGraph():
            ...
            staticVariables.aniObj = animation.FuncAnimation(staticVariables.fig, aniObj.animate, interval = 400)
            tk.mainloop()
...
class Animate(object):    #animate 6F object
    ...
    def animate(self, i): #i for 'interval'
        if staticVariables.isPaused == True:
            ...
            for x,y in staticVariables.xyList:
                xs.append(x)   
                ys.append(y)    
                ucl.append(ucl_val)
                lcl.append(lcl_val)
...
class AnimationCSV(Thread):
    ...
    def run(self):
        self.val = staticVariables.x
        while staticVariables.isStopped == False:
            print('x is ' + str(self.val))
            if self.count < 1:      #do this once the first time
                with open((staticVariables.localPath_temp), 'a', newline='') as csvfile:     #'a' for appending, 'w' for overriding (erase all then write) and 'r' for readonly
                    ...
                    writer.writerow({'x_axis': str(staticVariables.x), 'y_axis': str(staticVariables.y)})
                    self.val2 = self.val
                    self.count = self.count + 1
            elif self.count > 0 and self.val2  != staticVariables.x:    #dont append duplicate values
                with open((staticVariables.localPath_temp), 'a', newline='') as csvfile:     #'a' for appending, 'w' for overriding (erase all then write) and 'r' for readonly
                    fieldnames = ['x_axis', 'y_axis']   #x and y headers
                    writer = csv.DictWriter(csvfile, fieldnames=fieldnames) # DictWriter does same thing as a writerclass
                    #writer.writeheader()    #add column titles
...
class SerialCommunication(Thread):
    ...
    def run(self):    #disconnect during use
        count = 0
        arduino = None
        ...
        while staticVariables.isStopped == False:
            try: 
                if count < 1:   #if no initial connection error then only make exception error once
                    arduino = serial.Serial('COM6', 9600, timeout=.05)   #create connection object
                count = 1
                ...
                if count < 1 and (staticVariables.connectionError == False and staticVariables.notConnectionError == True) and (staticVariables.permissionError == False and staticVariables.notPermissionError == True): #if errored when trying to log in, once all good then make a connection.
                    arduino = serial.Serial('COM6', 9600, timeout=.05)
                time.sleep(1)
                #staticVariables.isPaused = False
                serialcmd = 'g210'
                ...
                while staticVariables.isStopped == False:
                    thStart = timeit.default_timer()
                    print('I am "SericalCom" and my Thread name is ' + self.getName() + ' my status is ' + str(self.is_alive()) + ' ' + str(thElapsed))
                    ...
                    with open(staticVariables.localPath, 'a', newline='') as csvfile:     #'a' for appending, 'w' for overriding (erase all then write) and 'r' for readonly
                        if len(response) > 0:
                            ...
                            staticVariables.xyList.append((x,y))
                    elapsed = (timeit.default_timer() - start_time)      
                    thElapsed = timeit.default_timer() - thStart     
                    x = x + elapsed             
                    #try:
                    #    extraWait = 0.5 - elapsed   #how much longer to wait to reach 0.5s
